...
export function fewestNbOfCubesEachColor(game: Game): Color {
	...
	for (const seq of game.sequences) {
		if (colorSet.red < seq.red) colorSet.red = seq.red;
		if (colorSet.green < seq.green) colorSet.green = seq.green;
		if (colorSet.blue < seq.blue) colorSet.blue = seq.blue;
	}
	return colorSet;
}

export function powerOfSet(colorSet: Color): number {
	return colorSet.red * colorSet.green * colorSet.blue;
}

export function sumOfPowerOfSets(games: Game[]) {
	let result = games
		.map((game) => fewestNbOfCubesEachColor(game))
		.map((set) => powerOfSet(set))
		.reduce((acc, current) => acc + current, 0);
	return result;
}

const result = sumOfPowerOfSets(games);
console.log("Sum of power of these sets : ", result);
